...
int main()
{
    const int N = 400;  //样本总数
    const int p = 6;    //参数个数：β0,Price,Income,Advertising,Good,Medium
    double *X  = (double*)malloc(N * p * sizeof(double));
    double *y  = (double*)malloc(N * sizeof(double));

    // ========== Carseats 400条数据 ==========
    int row = 0;
    //格式：X[row] = [1, Price, Income, Advertising, ShelveLoc_Good, ShelveLoc_Medium], y[row]=Sales
    //1
    ...
    printf("==== Carseats多元线性回归结果 ====\n");
    printf("截距 β0        = %.4f\n", beta[0]);
    printf("Price系数 β1   = %.4f\n", beta[1]);
    printf("Income系数 β2  = %.4f\n", beta[2]);
    printf("Advertising β3 = %.4f\n", beta[3]);
    printf("ShelveLoc_Good β4= %.4f\n", beta[4]);
    printf("ShelveLoc_Medium β5= %.4f\n", beta[5]);
